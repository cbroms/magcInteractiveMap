$(window).resize(function() {
  $(".faded-background").width(Math.ceil($(".main-image").width()));
  $(".shrink").width(Math.ceil($(".native").width()));

  $(".back-fill").width(Math.ceil($(".native").width()));

  // if window is small, set the width of images in the slider to be window size
  if (window.innerWidth < 520) {
    $(".slick-slide").css(
      "max-width",
      (window.innerWidth - 10).toString() + "px"
    );
  }
});

$(document).ready(function() {
  // button to close modal or return to map
  let currUrl = new URL(window.location.href);
  let step = currUrl.searchParams.get("step");
  let guide = currUrl.searchParams.get("guide");
  ...
  document.getElementById("closeButton").onclick = () => {
    try {
      ...
      }/map/?closed=${detail}&step=${step}&question-guide=${guide}`;
    }
  };
