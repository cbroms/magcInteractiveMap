...
$(document).ready(function() {
    ...
    mc.on("tap", function(e) {
        ...
        /***
        Button click functionality. If the user clicks within the valid region of the point 
        (check the click pos against each of the click locations), open details panel
        ***/
        $(".pulse-click").each(function() {
            ...
            if (
                xClick >= pos.x &&
                xClick <= pos.x + pos.width &&
                yClick >= pos.y &&
                yClick <= pos.y + pos.height
            ) {
                console.log("clicked");
                ...
                // if the iframe is not already loaded, add it
                if (iOS) {
                    // redirect to iOS details page
                    window.location.href = `details/${slug}/?step=${stepNumber}&guide=${guide}`;
                } else {
                    if (
                        !$(".int-" + slug)
                            .children()
                        ...
                        $(".int-" + slug).append(
                            '<iframe src="details/' +
                                slug +
                                last +
                                '" class="rounded details-frame" ></iframe>'
                        );
                    }
                    // open the modal
    ...
    panArea.on("transform", function(e) {
        ...
        // REMOVING this (5/13/19) as it results in poor performance
    ...
    panArea.zoomAbs(
        window.innerWidth / 2 - $(".zoomable").width() / 2,
        window.innerHeight / 2 - $(".zoomable").height() / 2,
        0.2
    );
    ...
    /***
    URL changes for tour funtionality 
    ***/
    ...
    if (guide) {
        // show the finish and continue buttons
        ...
        if (step != undefined && step < locationsJSON.length && step >= 0) {
            ...
            // step is unspecified, so set it to 0
            stepNumber = 0;
            ...
            window.history.replaceState(
                "",
                "",
                updateURLParameter(window.location.href, "step", "0")
            );
    ...
    if (demo) {
        ...
        $("#demo").on("hidden.bs.modal", function(e) {
            window.history.replaceState(
                "",
                "",
                updateURLParameter(window.location.href, "demo", "false")
            );
        });
    }
...
function incrementStep() {
    ...
    if (stepNumber + 1 >= locationsJSON.length) {
        ...
        window.history.replaceState(
            "",
            "",
            updateURLParameter(window.location.href, "step", "")
        );
        window.history.replaceState(
            "",
            "",
            updateURLParameter(window.location.href, "question-guide", "false")
        );
...
function skipNextStep() {
    ...
    if (stepNumber + 1 >= locationsJSON.length) {
        ...
        // add an event listener for the close of the target modal or return from details page
        if (iOS) {
            if (last === target) {
                $("#finishTour").fadeOut(600);
                $(".pulse").fadeIn(600);
